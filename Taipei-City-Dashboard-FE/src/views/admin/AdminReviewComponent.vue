...
function handleOpenSettings(component) {
	...
	dialogStore.showDialog("adminComponentReview");
}

function handleOpenReview(component) {
	adminStore.getComponentData(component);
      ...
      <tbody v-if="adminStore.submittedComponents.length !== 0">
        ...
        >
          <td class="admineditcomponent-table-settings">
			...
			<button @click="handleOpenReview(component)">
...
.admineditcomponent {
	...
	.admineditcomponent-table-settings {
		display: flex;
		align-items: center;
		justify-content: center;
		gap: 0.25rem;
