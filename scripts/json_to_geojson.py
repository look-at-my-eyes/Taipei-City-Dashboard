...
geojson_output_file = file_name + ".geojson"
...
for index, row in df.iterrows():
    try:
        ...
        lon_col = next(
            (col for col in lon_col_names if col in row and pd.notna(row[col])), None
        )
        lat_col = next(
            (col for col in lat_col_names if col in row and pd.notna(row[col])), None
        )
...
with open(
    f"../Taipei-City-Dashboard-FE/public/mapData/{geojson_output_file}",
    "w",
    encoding="utf-8",
) as f:
    json.dump(
